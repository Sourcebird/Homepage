...
if (isset($_POST['signupsubmit'])) {
    ...
    */
    ...
    if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {
        ...
        $_SESSION['ERRORS']['usernameerror'] = 'username has to be between 6 and 24 charakters';
        header("Location: ../");        
        exit();    
    } else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {

        $_SESSION['ERRORS']['usernameerror'] = 'username contains invalid symbols';
        header("Location: ../");
        exit();
        ...
        header("Location: ../");
        exit();
    } else if (!(strlen($password)>7 && strlen($password)<26)) {
        
        $_SESSION['ERRORS']['passworderror'] = 'please pick a password between 8 and 25 charakters';
        header("Location: ../");
        exit();
